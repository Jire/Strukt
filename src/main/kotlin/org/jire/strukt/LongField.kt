...
interface LongField : Field {
	...
	operator fun get(address: Long) = threadSafeType.run { readLong(pointer(address)) }
	operator fun set(address: Long, value: Long) = threadSafeType.run { writeLong(pointer(address), value) }
